...
class JSONResponseMixin(object):
    ...
    def convert_context_to_json(context):
        ...
        for trainingcenter in context['trainingcenters']:
            if not first_flag:
                result += ',\n'
            result += '    "%s" : "%s"' % (trainingcenter.id, trainingcenter.name)
            first_flag = False
        result += '\n}'
        return result
...
class JSONTrainingCenterListView(TrainingCenterMixin, JSONResponseMixin, ListView):
...
class TrainingCenterDetailView(TrainingCenterMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        Because Training Center slugs are unique within a Project, we need to make
        sure that we fetch the correct Training Center from the correct Project
...
class TrainingCenterDeleteView(
    ...
    def get_queryset(self):
        ...
        We need to filter the Training Center objects by Project before passing to
        get_object() to ensure that we return the correct Training Center object.
        The requesting User must be authenticated

        :returns: Training Center queryset filtered by Project
        :rtype: QuerySet
        :raises: Http404
        """
...
class TrainingCenterUpdateView(
    ...
    def get_form_kwargs(self):
        ...
        self.trainingcenter = TrainingCenter.objects.get(slug=training_center_slug)
        self.project_slug = self.kwargs.get('project_slug', None)
        self.project = Project.objects.get(slug=self.project_slug)
