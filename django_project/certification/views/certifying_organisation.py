...
from ..models import CertifyingOrganisation, TrainingCenter # noqa
...
class JSONResponseMixin(object):
    ...
    def convert_context_to_json(context):
        ...
        for certifyingorganisation in context['certifyingorganisations']:
            if not first_flag:
                result += ',\n'
            result += '    "%s" : "%s"' % (certifyingorganisation.id, certifyingorganisation.name)
            first_flag = False
        result += '\n}'
        return result
...
class JSONCertifyingOrganisationListView(CertifyingOrganisationMixin, JSONResponseMixin, ListView):
    """List view for Certifying Organisation as json object - needed by javascript."""
...
class CertifyingOrganisationDetailView(CertifyingOrganisationMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        :rtype: dict
        """
        context = super(CertifyingOrganisationDetailView, self).get_context_data(**kwargs)
        project_slug = self.kwargs.get('project_slug', None)
        context['project_slug'] = project_slug
        if project_slug:
            context['the_project'] = Project.objects.get(slug=project_slug)
...
class CertifyingOrganisationDeleteView(LoginRequiredMixin, CertifyingOrganisationMixin, DeleteView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return super(CertifyingOrganisationDeleteView, self).get(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.project_slug = self.kwargs.get('project_slug', None)
        self.project = Project.objects.get(slug=self.project_slug)
        return super(CertifyingOrganisationDeleteView, self).post(request, *args, **kwargs)
...
class CertifyingOrganisationCreateView(
    ...
    def get_context_data(self, **kwargs):
        ...
        :rtype: dict
        """
        context = super(CertifyingOrganisationCreateView, self).get_context_data(**kwargs)
...
# noinspection PyAttributeOutsideInit
class CertifyingOrganisationUpdateView(LoginRequiredMixin, CertifyingOrganisationMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        :rtype: dict
        """
        context = super(CertifyingOrganisationUpdateView, self).get_context_data(**kwargs)
        context['certifyingorganisations'] = self.get_queryset() \
            .filter(project=self.project)
        return context
...
class ApproveCertifyingOrganisationView(
    ...
    permanent = False
    query_string = True
    pattern_name = 'certifyingorganisation-create'

    def get_redirect_url(self, project_slug, slug):
        """Save Certifying Organisation as approved and redirect

        :param project_slug: The slug of the parent Certifying Organisation's parent Project
        ...
        certifyingorganisation = get_object_or_404(certifyingorganisation_qs, slug=slug)
